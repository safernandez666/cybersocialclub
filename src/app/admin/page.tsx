                  ...
                  <div className="p-5 pl-6">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2 lg:pt-1">
                        {filter === "pending_verification" && (<><button onClick={async () => { setProcessing(member.id); try { const res = await fetch(`/api/members/${member.id}/resend-verification`, { method: "POST", headers: authHeaders() }); if (res.ok) { alert("Email de verificación reenviado"); } else { const data = await res.json(); alert("Error: " + (data.error || "No se pudo reenviar")); } } catch { alert("Error al reenviar verificación"); } finally { setProcessing(null); } }} disabled={processing === member.id} className="flex items-center gap-2 rounded-full bg-blue-500/10 px-4 py-2 font-mono text-xs uppercase tracking-widest text-blue-400 transition-all hover:bg-blue-500/20 disabled:opacity-50"><Mail className="h-3.5 w-3.5" />Reenviar Verificación</button><button onClick={async () => { setProcessing(member.id); try { await fetch(`/api/admin/members/${member.id}`, { method: "DELETE", headers: authHeaders() }); setMembers((prev) => prev.filter((m) => m.id !== member.id)); fetchStats(); } catch { } finally { setProcessing(null); } }} disabled={processing === member.id} className="flex items-center gap-2 rounded-full bg-red-500/10 px-4 py-2 font-mono text-xs uppercase tracking-widest text-red-400 transition-all hover:bg-red-500/20 disabled:opacity-50"><X className="h-3.5 w-3.5" />Eliminar</button></>)}
